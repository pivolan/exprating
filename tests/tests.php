...
$strs =  [
    ['Прове рка была тут быть', 'Проерка была тут быть']

];
foreach($strs as $str){
    similar_text($str[0], $str[1], $result1);
    $result2 = levenshtein($str[0], $str[1]);
    evaltextRus(2, $str[0], $str[1], $result3);

    echo sprintf("r1: %s, r2: %s, r3: %s \n", $result1,
        $result2,
        $result3['sim']);
}
